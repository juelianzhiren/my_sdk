...
abstract class HandleHelper {
	...
	private static final String TAG = "childedu.HandleHelper";
    ...
    private boolean allowAdjustCoordinate = false; // 是否允许调整取景框
    ...
    public void setAllowAdjustCoordinate(boolean allowAdjustCoordinate) {
    	this.allowAdjustCoordinate = allowAdjustCoordinate;
    }
    ...
    void updateCropWindow(float x,
        ...
        // 选择区域根据用户选择而调整
        if (primaryEdge != null && allowAdjustCoordinate){
        	if(primaryEdge == Edge.TOP){
        		primaryEdge.adjustCoordinate(x, y, imageRect, snapRadius, UNFIXED_ASPECT_RATIO_CONSTANT);
        		primaryEdge.adjustCoordinate(x, y + (imageRect.bottom - imageRect.top), imageRect, snapRadius, UNFIXED_ASPECT_RATIO_CONSTANT);
        	}else if(primaryEdge == Edge.BOTTOM){
        		primaryEdge.adjustCoordinate(x, y, imageRect, snapRadius, UNFIXED_ASPECT_RATIO_CONSTANT);
        		primaryEdge.adjustCoordinate(x, y - (imageRect.bottom - imageRect.top), imageRect, snapRadius, UNFIXED_ASPECT_RATIO_CONSTANT);
        	}
        }
