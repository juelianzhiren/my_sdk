...
public class HorizontalEx extends ViewGroup {
    private static final String TAG = "noahedu.HorizontalEx";
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                mVelocityTracker.computeCurrentVelocity(1000, mConfiguration.getScaledMaximumFlingVelocity());
                float xVelocity = mVelocityTracker.getXVelocity();
                Log.v(TAG, "xVelocity = " + xVelocity + "; configuration.getScaledMinimumFlingVelocity() = " + mConfiguration.getScaledMinimumFlingVelocity() + "; scrollX = " + scrollX);
                if (Math.abs(xVelocity) > mConfiguration.getScaledMinimumFlingVelocity()) {
                    childIndex = xVelocity < 0 ? childIndex + 1 : childIndex - 1;
                } else {
                    Log.v(TAG, "<<, childIndex = " + childIndex + "; scrollX = " + scrollX + "; childWidth = " + childWidth);
                    childIndex = (scrollX + childWidth / 2) / childWidth;
                }
                Log.v(TAG, "childIindex 111 = " + childIndex);
                childIndex = Math.min(getChildCount() - 1, Math.max(childIndex, 0));
                Log.v(TAG, "childIindex 222 = " + childIndex);
    ...
    public void computeScroll() {
        Log.v(TAG, "computeScroll, mScroller.computeScrollOffset() = " + mScroller.computeScrollOffset());
